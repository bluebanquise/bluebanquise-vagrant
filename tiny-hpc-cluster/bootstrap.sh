...
/usr/bin/git clone https://github.com/bluebanquise/bluebanquise /etc/bluebanquise -b stable-1.2
...
if [[ ! -d /var/www/html/repositories/${DISTRIBUTION}/${MINOR_RELEASE}/${ARCH}/bluebanquise/ ]]; then
    (cd /var/www/html/repositories/${DISTRIBUTION}/${MINOR_RELEASE}/${ARCH}/ && /usr/bin/wget -q -rkp -l2 -np -nH -R "index.html*" -R "*.gif" --cut-dirs=5 https://bluebanquise.com/repository/releases/1.2/el${MAJOR_RELEASE}/${ARCH}/bluebanquise/)
fi
